...
def get_model(model_name, config_path="config/default.yaml", pretrained=True):
    ...
    if "dinov2" in model_name:
        ...
        model = timm.create_model(
            "vit_large_patch14_dinov2.lvd142m", pretrained=True, img_size=518
        )
        model = model.eval()
        ...
        if isinstance(model.head, nn.Identity):
            ...
            elif hasattr(model, "pre_logits") and hasattr(
                model.pre_logits, "out_features"
            ):
                in_features = model.pre_logits.out_features
            else:
                raise AttributeError(
                    "Cannot determine in_features for dinov2 head replacement."
                )
        ...
        if "convnext_base" in model_name:
            ...
            "densenet121" in model_name
            or "densenet169" in model_name
            or "densenet201" in model_name
            or "densenet161" in model_name
        ):
