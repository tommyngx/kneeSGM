...
def extract_real_id(image_name):
    return image_name[3:7]

def generate_dataset(input_folder, metadata_csv, split_csv, output_dir, data_name, seed):
    random.seed(seed)
    metadata = load_metadata(metadata_csv)
    split_data = load_split_csv(split_csv)
    ...
    metaraw_data = []
    val_ids = split_data[split_data['split'] == 'VAL']['ID'].apply(extract_real_id).tolist()
            ...
            if data_name == 'CGMH':
                ...
                if not row.empty:
                    ...
                    if kl_value is not None:
                        ...
                        split = 'TEST' if id_value in val_ids else 'TRAIN'
...
def main(input_folder, metadata_csv, split_csv, data_name, config='default.yaml', seed=2024):
    config_path = os.path.join('config', config)
    config = load_config(config_path)
    output_dir = os.path.join(config['output_dir'], 'yolo', 'runs', data_name)
    
    generate_dataset(input_folder, metadata_csv, split_csv, output_dir, data_name, seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--split_csv', type=str, required=True, help='Path to the split CSV file')
    ...
    main(args.input_folder, args.metadata_csv, args.split_csv, args.data_name, args.config, args.seed)
