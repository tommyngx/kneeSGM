...
def save_confusion_matrix(
    ...
    font_url = "https://raw.githubusercontent.com/tommyngx/style/main/Poppins.ttf"
    ...
    plt.xlabel(
        "Predicted", fontproperties=prop, fontsize=18
    ...
    if custom_title is not None:
        ...
        if epoch is not None:
            title += f" - Epoch {epoch}"
    plt.title(
        title, fontproperties=prop, fontsize=20, pad=20
    ...
    # Save the figure
    filename = (
        f"{filename_prefix}confusion_matrix.png"
        if epoch is None
        else f"{filename_prefix}confusion_matrix_epoch_{epoch}_acc_{acc:.4f}.png"
    )
...
def save_roc_curve(
    ...
    plt.plot(
        ...
        color="darkred",
        lw=2,
        label=f"AUC: {roc_auc * 100:.0f}% ({confidence_lower * 100:.0f}% - {confidence_upper * 100:.0f}%)",
    )
    ...
    plt.xticks(
        np.arange(0, 1.1, step=0.20),
        labels=[f"{int(x * 100)}%" for x in np.arange(0, 1.1, step=0.20)],
        fontsize=15,
    )
    plt.yticks(
        np.arange(0, 1.1, step=0.20),
        labels=[f"{int(y * 100)}%" for y in np.arange(0, 1.1, step=0.20)],
        fontsize=15,
    )
    ...
    fig.patch.set_facecolor(
        "white"
    )  # Set the background color outside the plot area to white
    ...
    filename = (
        f"{filename_prefix}roc_curve.png"
        if epoch is None
        else f"{filename_prefix}roc_curve_epoch_{epoch}_acc_{acc:.4f}.png"
    )
    plt.savefig(os.path.join(output_dir, filename))
    plt.close()
    ...
    saved_files = sorted(
        [f for f in os.listdir(output_dir) if f.startswith("roc_curve_epoch_")],
        key=lambda x: float(x.split("_acc_")[-1].split(".png")[0]),
        reverse=True,
    )
    for file in saved_files[3:]:
