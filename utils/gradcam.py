...
def generate_gradcam_vit(activations, gradients, image):
    ...
    if gradients.dim() == 2:
        ...
        gradients = gradients.expand(
            activations.size(0), activations.size(1), activations.size(2)
        )
    else:
...
def generate_gradcam_caformer(activations, gradients, image):
    ...
    if gradients.dim() == 2:
        ...
        if gradients.size(1) != activations.size(1) or gradients.size(
            2
        ) != activations.size(2):
            gradients = torch.mean(gradients, dim=1, keepdim=True)
            gradients = gradients.expand(-1, activations.size(1), activations.size(2))
...
def generate_gradcam_fastvit(activations, gradients, image):
    if activations.dim() == 4:
        ...
        if gradients.dim() == 4:
            gradients = torch.mean(gradients, dim=[2, 3])
        if gradients.dim() == 2:
            gradients = gradients.unsqueeze(1).expand(
                activations.size(0), activations.size(1), gradients.size(1)
            )
        elif gradients.dim() == 3 and gradients.size(1) == 1:
            gradients = gradients.expand(
                activations.size(0), activations.size(1), gradients.size(2)
            )
        ...
        pooled_gradients = torch.mean(gradients, dim=1, keepdim=True)
        pooled_gradients = pooled_gradients.expand(
            activations.size(0), activations.size(1), activations.size(2)
        )
        weighted_activations = activations * pooled_gradients
        heatmap = torch.sum(weighted_activations, dim=-1).squeeze()
...
def post_process_heatmap(heatmap, image):
    ...
    if isinstance(image, torch.Tensor):
        if image.dim() == 4:
            ...
            raise ValueError(
                "Unexpected image tensor shape for resizing GradCAM heatmap."
            )
...
def save_random_predictions(
    ...
    for i in range(4):
        ...
        if use_gradcam_plus_plus:
            heatmap = generate_gradcam_plus_plus(
                model, images[i].unsqueeze(0), target_layer, model_name
            )
        else:
        ...
        if i + 4 < len(images):
            ...
            if use_gradcam_plus_plus:
                heatmap = generate_gradcam_plus_plus(
                    model, images[i + 4].unsqueeze(0), target_layer, model_name
                )
            else:
                heatmap = generate_gradcam(
                    model, images[i + 4].unsqueeze(0), target_layer, model_name
    ...
    saved_files = sorted(
        [
            ...
            if f.startswith(f"random_predictions_{model_name}_")
        ],
        key=lambda x: (
            float(x.split("_acc_")[-1].split(".png")[0]) if "_acc_" in x else 0
        ),
        reverse=True,
    )
...
def get_target_layer(model, model_name):
    if "vit_base_patch16_224" in model_name:
        ...
        # Trả về layer convolution cuối cùng của VGG
        return model.features[-1]
    else:
        raise ValueError(f"Model {model_name} not supported for Grad-CAM.")
...
def plot_gradcam_on_image(
    ...
    try:
        if any(
            ...
            if (
                gradients.dim() == 4
                and activations.dim() == 4
                and gradients.shape[2:] == activations.shape[2:]
            ):
                heatmap = generate_gradcam_plus_plus_cnn(
                    activations, gradients, input_tensor
                )
            else:
                heatmap = generate_gradcam_cnn(activations, gradients, input_tensor)
        else:
            heatmap = generate_gradcam(
                model, input_tensor, target_layer, model_name=model_name
            )
